  ...
  try {
    ...
    const datasets = {
      ...
      current: [
        {
          label: 'Dòng điện A',
          data: data.map((row) => row.current1),
          borderColor: 'red',
          borderWidth: 2,
          fill: false,
          tension: 0.1,
        ...
        {
          label: 'Dòng điện B',
          data: data.map((row) => row.current2),
          borderColor: 'blue',
          borderWidth: 2,
          fill: false,
          tension: 0.1,
        ...
        {
          label: 'Dòng điện C',
          data: data.map((row) => row.current3),
          borderColor: 'green',
          borderWidth: 2,
          fill: false,
          tension: 0.1,
      ...
      powerFactor: [
        {
          label: 'Hệ số công suất tổng',
          data: data.map((row) => row.power_factor_total),
          borderColor: 'orange',
          borderWidth: 2,
          fill: false,
          tension: 0.1,
...
function updateCharts(labels, datasets, timestamps) {
  ...
  charts.powerFactor = createChart(
    'chartPowerFactor',
    labels,
    datasets.powerFactor,
    'Hệ số công suất',
    timestamps
  );
}
